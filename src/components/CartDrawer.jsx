...
export default function CartDrawer({ open, onClose }) {
  ...
  const handleQuantity = (id, quantity) => dispatch(setQuantity({ id, quantity }));
                ...
                <div className="px-3 md:px-6 py-4 border-t border-[#e5d3e9] bg-[#f9e7f6]">
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <span className="font-bold text-lg">
                      R${items.reduce((sum, item) => sum + item.price * item.quantity, 0).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}
                    </span>
                  </div>
                  <button
                    className="w-full bg-[#7a4fcf] hover:bg-[#ae95d9] text-white rounded-full py-3 font-medium transition"
                    // onClick={...} // Aqui você pode colocar a ação de finalizar compra
                  >
                    Iniciar Compra
                  </button>
                </div>
